...
\subsection*{思考方向：}
\begin{itemize}
    \item \textbf{在确定的式子减少未知数、结合题目需要的变量以明确关系}
    \item \textbf{通过隐藏细节，从整体比例找关系解复杂方程}
\end{itemize}
...
\subsection*{思考方向：}
\begin{itemize}
    \item \textbf{通过导数判断函数单调性与极值得到零点个数信息}
\end{itemize}

\newpage
\section{题目：导数与微分中值定理的综合应用}

\begin{enumerate}
    \item[2.(1)] 设在 $[0, 1]$ 上 $f''(x) > 0$，则 $f'(0), f'(1), f(1)-f(0)$ 或 $f(0)-f(1)$ 几个数的大小顺序为（ \quad ）.
...
\begin{itemize}
    \item 因为 $f''(x) > 0$，所以 $f'(x)$ 在 $[0, 1]$ 上单调递增。
    \item 由此可知 $f(x)$ 在 $[0, 1]$ 上连续且在 $(0, 1)$ 上可导。
\end{itemize}
...
由于$f'(x)$在$(0,1)$上单调递增，必有
\[ f'(1) > f(1)-f(0) > f'(0) \]
因此，答案为B。

\subsection*{思考方向：}
\begin{itemize}
    \item \textbf{拉格朗日中值定理}：存在 $\xi \in (0, 1)$，使得 $f(1) - f(0) = f'(\xi)(1 - 0) = f'(\xi)$。
    \item \textbf{二阶导数的意义}：若 $f''(x) > 0$，则一阶导数 $f'(x)$ 在区间内是单调递增的。
    \item \textbf{结合判断}：通过比较 $0 < \xi < 1$ 时导数值的大小关系，即可得出选项。
\end{itemize}

\newpage
\section{题目：高阶导数与函数性态判定}

\begin{enumerate}
    \item[2.(2)] 设 $f'(x_0) = f''(x_0) = 0, f'''(x_0) > 0$，则（ \quad ）.
    \begin{enumerate}[label=(\Alph*)]
...
\subsection*{解：}
已知$f'''(x_{0})>0$，则由$f''(x_{0})=0$可知，$f''(x)$在$x_{0}$某邻域单调递增，有
\begin{align*}
     &\lim_{x \to x_{0}^{+}} f''(x) > 0 \\
     &\lim_{x \to x_{0}^{-}} f''(x) < 0
\end{align*}
得到$(x_{0},f(x_0))$是$f(x)$的拐点,且$f'(x_{0})$是$f'(x)$的极小值，A排除.这意味着$f'(x)$始终$\ge 0$。 
\\

此时可推断出$f(x)$无极值点，B、C排除。 
\\

若能构造出具体函数亦可直接解出。如$f(x)=x^{3}$

\subsection*{思考方向：}
\begin{itemize}
    \item \textbf{ 紧密结合导数定义，逐步分析函数的单调性和凹凸性，最终得出结论。}
\end{itemize}
